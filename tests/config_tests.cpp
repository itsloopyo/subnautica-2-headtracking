...
// and the default hotkeys are the fleet's.
//
// sn2ht_config_tests --render-config <path> writes the rendered file to <path>
...
fs::path TempDir() {
    ...
    const fs::path dir = fs::path(temp) / ("sn2ht_config_tests_" + std::to_string(GetCurrentProcessId()));
    fs::remove_all(dir);
...
void RenderMatchesCommittedFile() {
    const std::string committed = ReadBytes(fs::path(SN2HT_SOURCE_DIR) / "HeadTracking.ini");
...
void DefaultRowsFollowDefaultsIni(const fs::path& dir) {
    ...
    Check(loaded.config.tooltip_follow_reticle && loaded.config.tooltip_follow_scale == 1.0f,
          "the game's own rows keep their values");
}
...
int main(int argc, char** argv) {
    ...
    if (argc != 1) {
        std::printf("usage: sn2ht_config_tests [--render-config <path>]\n");
        return 2;
    }
